...
module ActiveModel
  ...
  module Enum
    ...
    def _enum(name, values, prefix: nil, suffix: nil, scopes: false, **_options)
      ...
      attr = attr.attr if attr.is_a?(EnumType)
      EnumType.new(name, enum_values, attr)
    ...
    def detect_enum_conflict!(enum_name, method_name, klass_method = false)
      if !klass_method && method_defined_within?(method_name, _enum_methods_module, Module)
    ...
    def raise_conflict_error(enum_name, method_name, type: 'instance', source: 'Active Record')
      raise ArgumentError,
            format(ENUM_CONFLICT_MESSAGE, enum: enum_name, klass: name, type: type, method: method_name, source: source)
    end
    ...
    def detect_negative_enum_conditions!(method_names)
      ...
      method_names.select { |m| m.start_with?('not_') }.each do |potential_not|
        ...
        next unless method_names.include?(inverted_form)

        logger.warn "Enum element '#{potential_not}' in #{name} uses the prefix 'not_'." \
